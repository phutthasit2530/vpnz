...
if [ ! -f "$database" ]; then
	...
	while read _user; do
        ...
        lim=$(echo -e "\033[1;33mจำกัด\033[1;37m: $limit")
        printf '%-65s%s\n' "$l_user" "$lim"
		_userPass+="\n${_oP}:${_user}"
	done <<< "${_userT}"
	echo ""
    ...
    if [[ -z $option ]]; then
		...
		if cat /etc/passwd |grep -w $usuario > /dev/null; then
			echo -ne "\n\033[1;32mผู้ใช้ใหม่ จำกัด \033[1;33m$usuario\033[1;37m: "; read sshnum
			if [[ -z $sshnum ]]
			then
				...
				then
					...
					then
						...
						tput setaf 7 ; tput setab 4 ; tput bold ; echo "" ; echo "จำกัด การใช้กับผู้ใช้ $usuario foi $sshnum " ; tput sgr0
						sleep 2
						exit
					fi
				fi
			...
			tput setaf 7 ; tput setab 1 ; tput bold ; echo "" ; echo "ผู้ใช้ $usuario ไม่พบ" ; echo "" ; tput sgr0
			exit 1
		fi
	fi
fi
